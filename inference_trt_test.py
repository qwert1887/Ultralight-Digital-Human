    ...
    with open(engine_file_path, "rb") as f, trt.Runtime(TRT_LOGGER) as runtime:
        ...
        for binding in engine:
            ...
            if engine.get_tensor_mode(binding) == trt.TensorIOMode.INPUT:
                ...
                # host_memory = np.ascontiguousarray(input_item.astype(np.float32))

                # print(f"host_memory.shape:{host_memory.shape}, {host_memory.dtype}")
                # cuda_memory = cuda.mem_alloc(host_memory.nbytes)
                # cuda_inputs.append(cuda_memory)
                # host_inputs.append(host_memory)
                # bindings.append(int(cuda_memory))
            else:
                # host_output = cuda.pagelocked_empty(size, np.float32)
                # cuda_memory = cuda.mem_alloc(host_output.nbytes)
