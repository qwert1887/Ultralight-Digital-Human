...
from inference_trt_v2 import TRTEngine
...
# cv2.resizeWindow('Display_Window', 720, 1280)
...
for i in tqdm(range(audio_feats.shape[0]), total=audio_feats.shape[0], ncols=100, desc="Generating"):
    ...
    img_concat_T = np.concatenate((img_real_ex/255.0, img_masked/255.0), axis=0, dtype=np.float32)
    # img_concat_T = img_concat_T.astype(np.float32)
    audio_feat = get_audio_features(audio_feats, i)
    if mode=="hubert":
        audio_feat = audio_feat.reshape(1,32,32,32)
    elif mode=="wenet":
        audio_feat = audio_feat.reshape(1,128,16,32)  # (256,16,32)
    ...
    trt_out = engine.do_inference(img_concat_T, audio_feat)
    ...
    # sys.exit()
    crop_img_ori[4:164, 4:164] = trt_out
